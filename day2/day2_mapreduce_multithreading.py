...
class MapReduce(threading.Thread):
    ...
    def mapping(self, line):
        temp_list = []
        ss = re.sub(re.compile(r"[-:_!?',;.\'s]"), '', line.strip()).split(
...
def reduce(worddict):
    ...
    for key, value in worddict.items():
        if key not in res_dict:
            res_dict[key] = len(worddict[key])  # 对每个单词的词频列表求和，统计总的出现次数
    res_list = sorted(res_dict.items(), key=lambda item: item[1], reverse=True)  # 按照出现的频次进行排序
    print("================打印出单词出现的词频高于100次的情况（从多到少）====================================")
